...
if (
      isset($_POST['email']) and !is_null($_POST['email'])
      and isset($_POST['first_name']) and !is_null($_POST['first_name'])
      and isset($_POST['last_name']) and !is_null($_POST['last_name'])
      and isset($_POST['password']) and !is_null($_POST['password'])
      and isset($_POST['confirm']) and !is_null($_POST['confirm'])
) {
      if ($_POST['password'] === $_POST['confirm']) {
            if (strlen($_POST['password']) > 5) {
                  ...
                  if ($user === false) {
                        ...
                        $error = '.ایمیل وارد شده  قبلا ثبت نام کرده است';
                  }
            } else {
                  $error = '.رمزعبور حداقل 6 کاراکتر دارد';
            }
      } else {
            $error = '...عدم تطابق رمز عبور با تاییدیه';
      }
}else{
      if(!empty($_POST))
            $error = 'پرکردن فیلدها ضروری است ';
}
?>
<!DOCTYPE html>
<html lang="en">
